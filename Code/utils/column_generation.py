...
#     z_vars = {C: model.add_var(var_type=CONTINUOUS, lb=0, name=f"z_{C}") for C in w_c_dict.keys()}
...
#     AP-QP の整数線形計画を解く関数
#     """
#     edges = graph.generate_edges(adj_matrix_positive, adj_matrix_negative)
...
#     debug_term = sum(
#         dual_variables[v] * x_vars[u].x * x_vars[v].x
#         for u in vertices
#         for v in vertices
#         if x_vars[u].x is not None and x_vars[v].x is not None
#     )
#     print(f"Dual-related term in AP-QP: {debug_term}")
#     print(f"Normalization factor: {sum(x_vars[u].x for u in vertices)}")
...
#         print("AP-QP Solution:", solution_ap_qp)
...
def column_generation(vertices, A_plus, A_minus, D_plus, D_minus, lambda_val, init_partitions): 
    ...
    while(True):
        # LP(S)を解く lp_opt, lp_primal_sol, lp_dual_solの取得

        # AP-QPを解く ap_qp_opt, ap_qp_sol

        # if ap_qp_opt <= 0 なら stop

        # ap_qp_solよりS, w_C_dictの更新
        break